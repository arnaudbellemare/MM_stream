...
def robust_vol_calc(price, vol_days=35, annualized=True, timeframe='1d'):
    ...
    This function MUST be defined before ewmac_calc_vol.
    """
    log_returns = np.log(price / price.shift(1))
    min_p = max(2, int(vol_days / 2))
        ...
        if not watchlist_symbols:
            ...
            if df_watchlist.empty:
                ...
                with col2:
                    ...
                    fig_donut.add_annotation(text="<b>PERMUTATION</b><br>RESEARCH", x=0.5, y=0.5, xref="paper", yref="paper", showarrow=False, font=dict(size=12, color="#2c3e50"), align="center")
                ...
                if not df_watchlist.empty:
                    ...
                    fig_pca_quadrant.add_annotation(text="<b>Overall Laggards</b>", xref="paper", yref="paper", x=0.02, y=0.02, showarrow=False, align="left", font=dict(color="grey", size=11))
                    fig_pca_quadrant.add_annotation(text="<b>PERMUTATION RESEARCH ©</b>", xref="paper", yref="paper", x=0.5, y=0.5, showarrow=False, font=dict(size=64, color="rgba(220, 220, 220, 0.2)"), align="center")
                    fig_pca_quadrant.update_traces(textposition='top center', textfont_size=10)
                    fig_pca_quadrant.update_yaxes(title_text="Principal Component 2", zeroline=False)
