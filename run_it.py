...
# ==============================================================================
    ...
    # ==============================================================================
    # ==============================================================================
    # [NEW & CORRECTLY INDENTED] HELPER FUNCTION FOR CREATING SEQUENTIAL FEATURES
    # ==============================================================================
    def create_sequential_features(df_features, window_size):
        """
        Transforms a DataFrame of features into a sequential, flattened format for an MLP.
        """
        sequential_dfs = []
        for i in range(window_size):
            # Shift the data back by 'i' periods
            shifted_df = df_features.shift(i)
            # Rename columns to indicate the time lag
            shifted_df.columns = [f'{col}_lag_{i}' for col in df_features.columns]
            sequential_dfs.append(shifted_df)
        
        # Concatenate all shifted dataframes horizontally
        sequential_data = pd.concat(sequential_dfs, axis=1)
        
        # Drop rows with NaN values created by the shifting process
        return sequential_data.dropna()
    # ==============================================================================
    # FULL AND COMPLETE generate_comprehensive_watchlist FUNCTION 
    # [UPGRADED WITH SEQUENTIAL FEATURES & CORRECTLY INDENTED]
    # ==============================================================================
    ...
    def generate_comprehensive_watchlist(symbols, timeframe, limit):
        ...
        # Import Keras components inside the function
        import tensorflow as tf
        from tensorflow import keras
        from keras.models import Model
        from keras.layers import Input, Dense
        ...
        # Helper function to build the autoencoder
        def create_autoencoder(input_dim, encoding_dim=8):
            input_layer = Input(shape=(input_dim,))
        ...
        for i, symbol in enumerate(symbols):
            try:
                ...
                if df.empty or len(df) < 300: continue # Increased data requirement for windowing
                ...
                if len(features_df) < 150: continue # Need enough data for the window
                ...
                # --- 4. [NEW] CREATE SEQUENTIAL FEATURES ---
                st.info(f"[{symbol}] Creating 100-day sequential feature vectors...")
                WINDOW_LENGTH = 100
                sequential_features_df = create_sequential_features(encoded_features_df, WINDOW_LENGTH)
                
                # --- 5. TRIPLE-BARRIER LABELING ---
                VOL_MULT_PARAM = 1.5
                labels, volatility = get_triple_barrier_labels_and_vol(df_mlp['high'], df_mlp['low'], df_mlp['close'], 
                                                                        lookahead_periods=5, vol_mult=VOL_MULT_PARAM)
                
                # --- 6. DATA ALIGNMENT (with new sequential features) ---
                common_index = sequential_features_df.index.intersection(labels.index)
                final_features = sequential_features_df.loc[common_index]
                ...
                def tbl_score_func(y_true, y_pred, vol_series, vol_mult):
                    ...
                    lambda_param = 20.0
                    dcc_mask = (y_pred == y_true) & (y_true != 0); dic_mask = (y_pred != y_true) & (y_pred != 0) & (y_true != 0)
                ...
                # --- 8. GridSearchCV (with stronger regularization) ---
                st.info(f"[{symbol}] Running GridSearchCV on sequential features...")
                pipeline = make_pipeline(StandardScaler(), MLPClassifier(max_iter=500, random_state=42, early_stopping=True, activation='relu'))
                
                # [MODIFIED] Added simpler model and stronger regularization (alpha=0.01)
                # to combat overfitting with the new high-dimensional features.
                param_grid = {
                    'mlpclassifier__hidden_layer_sizes': [(16, 8), (32, 16)], 
